...
static void on_write(ble_ant_id_t * p_ant_id, ble_evt_t const * p_ble_evt)
{
  ...
  // if ((p_evt_write->handle == p_ant_id->led_char_handles.value_handle)
  //   && (p_evt_write->len == 1)
  //   && (p_ant_id->led_write_handler != NULL))
  // {
...
uint32_t ble_ant_id_init(ble_ant_id_t * p_ant_id, const ble_ant_id_init_t * p_ant_id_init)
{
  ...
  uint32_t ble_and_id_on_change(uint16_t conn_handle, ble_ant_id_t * p_ant_id, uint8_t button_state);
