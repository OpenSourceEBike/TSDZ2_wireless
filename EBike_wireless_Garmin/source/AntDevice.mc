using Toybox.System;
using Toybox.Ant;

class AntDevice extends Ant.GenericChannel {
    // settings
    static const NETWORK_TYPE = Ant.NETWORK_PUBLIC;
    static const CHANNEL_TYPE = Ant.CHANNEL_TYPE_RX_ONLY;
    static const CHANNEL_FREQUENCY = 71;
    static const CHANNEL_PERIOD = 8192;
    static const DEVICE_TYPE = 2;

    // for finding devices
    static const SEARCH_TRANSMISSION_TYPE = 1;

    // device managment
    hidden var _channel;
    hidden var _device_number = 35 + fieldDataID;
    hidden var _device;

    // timers for self-checking
    hidden var _lastMessageTime = 0;
    var timer = 0;
    var timerOnTimer = 0;
    var timerReopen = 0;
    var validData = false;

  function initialize()
  {
      open();
      timerOnTimer = 10;
  }

    // Open Connection. This is done automatically on initialisation
    // Usage: sensor.open();
    function open()
    {
        // Get the channel
        _channel = new Ant.ChannelAssignment(
            CHANNEL_TYPE,
            NETWORK_TYPE);

        // Set the configuration
        _device = new Ant.DeviceConfig({
          :deviceNumber => _device_number,
          :deviceType => DEVICE_TYPE,
          :transmissionType => 1,
          :messagePeriod => CHANNEL_PERIOD,
          :radioFrequency => CHANNEL_FREQUENCY,
          :searchTimeoutLowPriority => 12, // 30 seconds
          :searchThreshold => 0, // Pair to all transmitting sensors
          });


        GenericChannel.initialize(method(:onMessage), _channel);

        //Pair to all transmitting sensors
        GenericChannel.setDeviceConfig(_device);

        // Open the channel
        GenericChannel.open();
    }

    // Close connection. This is called when the object is destroyed
    // usage: sensor.close();
    function close()
    {
        GenericChannel.close();
        GenericChannel.release();
    }


    // Re-open connection. Use this when sensor is not responding
    // Usage: sensor.reopen();
    var reopening = false;
    function reopen() {
        if (!reopening) {
            close();
            timerOnTimer = 0;
            timerReopen = 5;
            reopening = true;
        } else {
        }
    }

    function reopen_finish () {
        open();
        reopening = false;
        timerOnTimer = 10;
        timerReopen = 0;
    }

    // execute a timer to check the activity of the sensor. Reset if there is a lapse.
    function onTimer() {
        var secondsSinceLastUpdate = timer - _lastMessageTime;
        if (secondsSinceLastUpdate > 30) {
            reopen();
            _lastMessageTime = timer;
            validData = false;
        }

        timerOnTimer = 10;
    }

    // Receive ANT message, used internally.
    function onMessage(msg)
    {
        // Parse the payload
        var payload = msg.getPayload();

        _lastMessageTime = timer;

        if (Ant.MSG_ID_BROADCAST_DATA == msg.messageId)
        {
            validData = true;

         switch (fieldDataID) {
            case 1:
            case 4:
              fieldData = (payload[0] | (payload[1] << 8)) / 10.0;
              break;

            case 2:
            case 5:
              fieldData = payload[0] / 5;
              break;

            case 3:
            case 7:
            case 9:
            case 10:
            case 12:
              fieldData = payload[0];
              break;

            case 6:
            case 8:
            case 11:
              fieldData = (payload[0] | (payload[1] << 8));
              break;

            case 13:
              fieldData = (payload[0] | (payload[1] << 8) | (payload[2] << 16) | (payload[3] << 24)) / 10;
              break;
            }

        }
        else if( Ant.MSG_ID_CHANNEL_RESPONSE_EVENT == msg.messageId )
        {
            if( Ant.MSG_ID_RF_EVENT == payload[0] )
            {
                if (payload[1] == Ant.MSG_CODE_EVENT_CHANNEL_CLOSED)
                {
                    // Channel closed, re-open
                    reopen();
                    validData = false;
                }
                else if (payload[1] == Ant.MSG_CODE_EVENT_RX_FAIL_GO_TO_SEARCH)
                {
                    // Channel dropped, re-open
                    reopen();
                    validData = false;
                }
                else if (payload[1] == Ant.MSG_CODE_EVENT_RX_SEARCH_TIMEOUT)
                {
                    // Search timed out, re-open
                    reopen();
                    validData = false;
                }
            }
        }
    }

  function getData() {
    return fieldData;
  }

  function timerTick() {
    timer++;

    if (timerOnTimer > 0) {
        timerOnTimer--;
    } else if (timerOnTimer == 0) {
        onTimer();
        timerOnTimer = -1;
    }

    if (timerReopen > 0) {
        timerReopen--;
    } else if (timerReopen == 0) {
        reopen();
        timerReopen = -1;
    }
  }
}
